    ...
    ui(new Ui::Widget)
{
    ...
    QSize imageSize = QSize(kuas.width(), kuas.height());//設定影像大小
    for(int i=0;i<=7;++i)kuasImages.append(QImage(imageSize, QImage::Format_Mono));//8張1位元影像
...
void Widget::on_pushButton_clicked()
{
    QString file=QFileDialog::getOpenFileName(this,"選擇圖檔");//選擇圖片
        ...
        ui->label->setGeometry(20,80,srcimg.width(),srcimg.height());
        ui->label->setPixmap(QPixmap::fromImage(srcimg));
        QSize imageSize = QSize(srcimg.width(), srcimg.height());
        for(int i=0;i<=7;++i)myImages.append(QImage(imageSize, QImage::Format_Mono));//8張1位元影像
        slice();
        water();
}
void Widget::slice()
...
double Widget::psnr()
{
    ...
    for(int j=0; j<srcimg.height(); j++){
       for(int i=0; i<srcimg.width(); i++){
           Gr=qRed(G.pixel(i,j));
           Sr=qRed(srcimg.pixel(i,j));
           temp=((Gr-Sr)*(Gr-Sr));
           MSE+=temp;
       }
    }
    MSE=MSE/(srcimg.height()*srcimg.width());
